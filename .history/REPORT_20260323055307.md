...
This project is the part of "WEB APPLICATION DEVELOPMENT" subject. Created by Weerapat Sangarun(66010781)
...
### +10 pts: Data Persistence (File/Database)
Notes are persisted to a real cloud database — PocketHost (hosted PocketBase) — meaning all notes survive server restarts, redeployments, and cold starts completely. This goes beyond the basic .json file requirement: data is stored in a structured NoSQL database with full CRUD support, accessible via REST API.

### +10 pts: Vercel Cloud Deployment with HTTPS

The application is deployed as two separate Vercel projects:

- **Backend API:** https://secure-note-app-lz21.vercel.app
...
**Backend** is deployed as a Vercel Serverless Function. `vercel.json` routes all requests to `server.js`, and Vercel wraps it as a serverless handler instead of a persistent process. The `module.exports = app` export is used when `process.env.VERCEL` is detected, replacing the normal `app.listen()` call.

**Frontend** is deployed as a Vite static build. Vercel runs `vite build` which bundles all React components into static HTML/CSS/JS files in the `dist/` folder. The `vercel.json` rewrite rule sends all routes to `index.html`, enabling React Router-style client-side navigation.

#### Why HTTPS matters here

Both deployments are served over **HTTPS automatically by Vercel** via TLS certificates. This means:

- The `Authorization` header containing `SECRET_TOKEN` is **encrypted in transit** between the user's browser and the backend server — no one can intercept it.
- Without HTTPS (plain HTTP), any network observer (ISP, Wi-Fi hotspot, etc.) could read the token and impersonate the user.
- The browser's `fetch()` API on modern sites also blocks mixed-content requests (HTTPS page calling HTTP API), so HTTPS on the backend is required for the frontend to work in production.